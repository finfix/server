...
// CheckAccess provides a mock function with given fields: ctx, accountGroupIDs, transactionIDs
func (_m *MockTransactionRepository) CheckAccess(ctx context.Context, accountGroupIDs []uint32, transactionIDs []uint32) error {
	ret := _m.Called(ctx, accountGroupIDs, transactionIDs)
	...
	if rf, ok := ret.Get(0).(func(context.Context, []uint32, []uint32) error); ok {
		r0 = rf(ctx, accountGroupIDs, transactionIDs)
	} else {
		r0 = ret.Error(0)
	}
