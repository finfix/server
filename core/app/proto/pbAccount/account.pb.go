// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.28.1
// 	protoc        v4.24.4
// source: account.proto

package pbAccount

import (
	pbEnum "core/app/proto/pbEnum"
	pbDatetime "pkg/proto/pbDatetime"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	emptypb "google.golang.org/protobuf/types/known/emptypb"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type GetAccountGroupsReq struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	UserID          uint32   `protobuf:"varint,1,opt,name=userID,proto3" jsonapi:"userID,omitempty"`                          // Идентификатор пользователя
	AccountGroupIDs []uint32 `protobuf:"varint,3,rep,packed,name=accountGroupIDs,proto3" jsonapi:"accountGroupIDs,omitempty"` // Идентификаторы групп счетов
}

func (x *GetAccountGroupsReq) Reset() {
	*x = GetAccountGroupsReq{}
	if protoimpl.UnsafeEnabled {
		mi := &file_account_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *GetAccountGroupsReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetAccountGroupsReq) ProtoMessage() {}

func (x *GetAccountGroupsReq) ProtoReflect() protoreflect.Message {
	mi := &file_account_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetAccountGroupsReq.ProtoReflect.Descriptor instead.
func (*GetAccountGroupsReq) Descriptor() ([]byte, []int) {
	return file_account_proto_rawDescGZIP(), []int{0}
}

func (x *GetAccountGroupsReq) GetUserID() uint32 {
	if x != nil {
		return x.UserID
	}
	return 0
}

func (x *GetAccountGroupsReq) GetAccountGroupIDs() []uint32 {
	if x != nil {
		return x.AccountGroupIDs
	}
	return nil
}

type GetAccountGroupsRes struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	AccountGroups []*AccountGroup `protobuf:"bytes,1,rep,name=accountGroups,proto3" jsonapi:"accountGroups,omitempty"` // Список групп счетов
}

func (x *GetAccountGroupsRes) Reset() {
	*x = GetAccountGroupsRes{}
	if protoimpl.UnsafeEnabled {
		mi := &file_account_proto_msgTypes[1]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *GetAccountGroupsRes) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetAccountGroupsRes) ProtoMessage() {}

func (x *GetAccountGroupsRes) ProtoReflect() protoreflect.Message {
	mi := &file_account_proto_msgTypes[1]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetAccountGroupsRes.ProtoReflect.Descriptor instead.
func (*GetAccountGroupsRes) Descriptor() ([]byte, []int) {
	return file_account_proto_rawDescGZIP(), []int{1}
}

func (x *GetAccountGroupsRes) GetAccountGroups() []*AccountGroup {
	if x != nil {
		return x.AccountGroups
	}
	return nil
}

type AccountGroup struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	ID           uint32 `protobuf:"varint,1,opt,name=ID,proto3" jsonapi:"ID,omitempty"`                     // Идентификатор группы счетов
	Name         string `protobuf:"bytes,2,opt,name=name,proto3" jsonapi:"name,omitempty"`                  // Название группы счетов
	Currency     string `protobuf:"bytes,3,opt,name=currency,proto3" jsonapi:"currency,omitempty"`          // Валюта группы счетов
	SerialNumber uint32 `protobuf:"varint,4,opt,name=serialNumber,proto3" jsonapi:"serialNumber,omitempty"` // Порядковый номер группы счетов
	Visible      bool   `protobuf:"varint,5,opt,name=visible,proto3" jsonapi:"visible,omitempty"`           // Видимость группы счетов
}

func (x *AccountGroup) Reset() {
	*x = AccountGroup{}
	if protoimpl.UnsafeEnabled {
		mi := &file_account_proto_msgTypes[2]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *AccountGroup) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AccountGroup) ProtoMessage() {}

func (x *AccountGroup) ProtoReflect() protoreflect.Message {
	mi := &file_account_proto_msgTypes[2]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AccountGroup.ProtoReflect.Descriptor instead.
func (*AccountGroup) Descriptor() ([]byte, []int) {
	return file_account_proto_rawDescGZIP(), []int{2}
}

func (x *AccountGroup) GetID() uint32 {
	if x != nil {
		return x.ID
	}
	return 0
}

func (x *AccountGroup) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *AccountGroup) GetCurrency() string {
	if x != nil {
		return x.Currency
	}
	return ""
}

func (x *AccountGroup) GetSerialNumber() uint32 {
	if x != nil {
		return x.SerialNumber
	}
	return 0
}

func (x *AccountGroup) GetVisible() bool {
	if x != nil {
		return x.Visible
	}
	return false
}

type QuickStatisticReq struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	UserID uint32 `protobuf:"varint,1,opt,name=userID,proto3" jsonapi:"userID,omitempty"` // Идентификатор пользователя
}

func (x *QuickStatisticReq) Reset() {
	*x = QuickStatisticReq{}
	if protoimpl.UnsafeEnabled {
		mi := &file_account_proto_msgTypes[3]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *QuickStatisticReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QuickStatisticReq) ProtoMessage() {}

func (x *QuickStatisticReq) ProtoReflect() protoreflect.Message {
	mi := &file_account_proto_msgTypes[3]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use QuickStatisticReq.ProtoReflect.Descriptor instead.
func (*QuickStatisticReq) Descriptor() ([]byte, []int) {
	return file_account_proto_rawDescGZIP(), []int{3}
}

func (x *QuickStatisticReq) GetUserID() uint32 {
	if x != nil {
		return x.UserID
	}
	return 0
}

type QuickStatisticRes struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	QuickStatistic []*QuickStatistic `protobuf:"bytes,1,rep,name=quickStatistic,proto3" jsonapi:"quickStatistic,omitempty"` // Краткая статистика
}

func (x *QuickStatisticRes) Reset() {
	*x = QuickStatisticRes{}
	if protoimpl.UnsafeEnabled {
		mi := &file_account_proto_msgTypes[4]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *QuickStatisticRes) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QuickStatisticRes) ProtoMessage() {}

func (x *QuickStatisticRes) ProtoReflect() protoreflect.Message {
	mi := &file_account_proto_msgTypes[4]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use QuickStatisticRes.ProtoReflect.Descriptor instead.
func (*QuickStatisticRes) Descriptor() ([]byte, []int) {
	return file_account_proto_rawDescGZIP(), []int{4}
}

func (x *QuickStatisticRes) GetQuickStatistic() []*QuickStatistic {
	if x != nil {
		return x.QuickStatistic
	}
	return nil
}

type QuickStatistic struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	TotalRemainder float64 `protobuf:"fixed64,1,opt,name=totalRemainder,proto3" jsonapi:"totalRemainder,omitempty"` // Общая сумма видимых счетов
	TotalExpense   float64 `protobuf:"fixed64,2,opt,name=totalExpense,proto3" jsonapi:"totalExpense,omitempty"`     // Общая сумма расходов
	AccountGroupID uint32  `protobuf:"varint,3,opt,name=accountGroupID,proto3" jsonapi:"accountGroupID,omitempty"`  // Идентификатор группы счетов
	TotalBudget    float64 `protobuf:"fixed64,4,opt,name=totalBudget,proto3" jsonapi:"totalBudget,omitempty"`       // Общая сумма расходов
	Currency       string  `protobuf:"bytes,5,opt,name=currency,proto3" jsonapi:"currency,omitempty"`               // Валюта
}

func (x *QuickStatistic) Reset() {
	*x = QuickStatistic{}
	if protoimpl.UnsafeEnabled {
		mi := &file_account_proto_msgTypes[5]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *QuickStatistic) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QuickStatistic) ProtoMessage() {}

func (x *QuickStatistic) ProtoReflect() protoreflect.Message {
	mi := &file_account_proto_msgTypes[5]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use QuickStatistic.ProtoReflect.Descriptor instead.
func (*QuickStatistic) Descriptor() ([]byte, []int) {
	return file_account_proto_rawDescGZIP(), []int{5}
}

func (x *QuickStatistic) GetTotalRemainder() float64 {
	if x != nil {
		return x.TotalRemainder
	}
	return 0
}

func (x *QuickStatistic) GetTotalExpense() float64 {
	if x != nil {
		return x.TotalExpense
	}
	return 0
}

func (x *QuickStatistic) GetAccountGroupID() uint32 {
	if x != nil {
		return x.AccountGroupID
	}
	return 0
}

func (x *QuickStatistic) GetTotalBudget() float64 {
	if x != nil {
		return x.TotalBudget
	}
	return 0
}

func (x *QuickStatistic) GetCurrency() string {
	if x != nil {
		return x.Currency
	}
	return ""
}

type SwitchReq struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	ID1    uint32 `protobuf:"varint,1,opt,name=ID1,proto3" jsonapi:"ID1,omitempty"`       // Идентификатор первого счета
	ID2    uint32 `protobuf:"varint,2,opt,name=ID2,proto3" jsonapi:"ID2,omitempty"`       // Идентификатор второго счета
	UserID uint32 `protobuf:"varint,3,opt,name=userID,proto3" jsonapi:"userID,omitempty"` // Идентификатор пользователя
}

func (x *SwitchReq) Reset() {
	*x = SwitchReq{}
	if protoimpl.UnsafeEnabled {
		mi := &file_account_proto_msgTypes[6]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *SwitchReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SwitchReq) ProtoMessage() {}

func (x *SwitchReq) ProtoReflect() protoreflect.Message {
	mi := &file_account_proto_msgTypes[6]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SwitchReq.ProtoReflect.Descriptor instead.
func (*SwitchReq) Descriptor() ([]byte, []int) {
	return file_account_proto_rawDescGZIP(), []int{6}
}

func (x *SwitchReq) GetID1() uint32 {
	if x != nil {
		return x.ID1
	}
	return 0
}

func (x *SwitchReq) GetID2() uint32 {
	if x != nil {
		return x.ID2
	}
	return 0
}

func (x *SwitchReq) GetUserID() uint32 {
	if x != nil {
		return x.UserID
	}
	return 0
}

type DeleteReq struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	ID     uint32 `protobuf:"varint,1,opt,name=ID,proto3" jsonapi:"ID,omitempty"`         // Идентификатор счета
	UserID uint32 `protobuf:"varint,2,opt,name=userID,proto3" jsonapi:"userID,omitempty"` // Идентификатор пользователя
}

func (x *DeleteReq) Reset() {
	*x = DeleteReq{}
	if protoimpl.UnsafeEnabled {
		mi := &file_account_proto_msgTypes[7]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *DeleteReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteReq) ProtoMessage() {}

func (x *DeleteReq) ProtoReflect() protoreflect.Message {
	mi := &file_account_proto_msgTypes[7]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteReq.ProtoReflect.Descriptor instead.
func (*DeleteReq) Descriptor() ([]byte, []int) {
	return file_account_proto_rawDescGZIP(), []int{7}
}

func (x *DeleteReq) GetID() uint32 {
	if x != nil {
		return x.ID
	}
	return 0
}

func (x *DeleteReq) GetUserID() uint32 {
	if x != nil {
		return x.UserID
	}
	return 0
}

type CreateReq struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Budget               float64            `protobuf:"fixed64,1,opt,name=budget,proto3" jsonapi:"budget,omitempty"`                             // Месячный бюджет
	Remainder            float64            `protobuf:"fixed64,2,opt,name=remainder,proto3" jsonapi:"remainder,omitempty"`                       // Остаток средств на счету
	Name                 string             `protobuf:"bytes,3,opt,name=name,proto3" jsonapi:"name,omitempty"`                                   // Название счета
	IconID               uint32             `protobuf:"varint,4,opt,name=iconID,proto3" jsonapi:"iconID,omitempty"`                              // Идентификатор иконки
	Type                 pbEnum.AccountType `protobuf:"varint,5,opt,name=type,proto3,enum=enums.AccountType" jsonapi:"type,omitempty"`           // Тип счета
	Currency             string             `protobuf:"bytes,6,opt,name=currency,proto3" jsonapi:"currency,omitempty"`                           // Валюта счета
	AccountGroupID       uint32             `protobuf:"varint,7,opt,name=accountGroupID,proto3" jsonapi:"accountGroupID,omitempty"`              // Группа счета
	Accounting           bool               `protobuf:"varint,8,opt,name=accounting,proto3" jsonapi:"accounting,omitempty"`                      // Подсчет суммы счета в статистике
	UserID               uint32             `protobuf:"varint,9,opt,name=userID,proto3" jsonapi:"userID,omitempty"`                              // Идентификатор пользователя
	GradualBudgetFilling bool               `protobuf:"varint,10,opt,name=gradualBudgetFilling,proto3" jsonapi:"gradualBudgetFilling,omitempty"` // Постепенное пополнение бюджета
}

func (x *CreateReq) Reset() {
	*x = CreateReq{}
	if protoimpl.UnsafeEnabled {
		mi := &file_account_proto_msgTypes[8]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CreateReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateReq) ProtoMessage() {}

func (x *CreateReq) ProtoReflect() protoreflect.Message {
	mi := &file_account_proto_msgTypes[8]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateReq.ProtoReflect.Descriptor instead.
func (*CreateReq) Descriptor() ([]byte, []int) {
	return file_account_proto_rawDescGZIP(), []int{8}
}

func (x *CreateReq) GetBudget() float64 {
	if x != nil {
		return x.Budget
	}
	return 0
}

func (x *CreateReq) GetRemainder() float64 {
	if x != nil {
		return x.Remainder
	}
	return 0
}

func (x *CreateReq) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *CreateReq) GetIconID() uint32 {
	if x != nil {
		return x.IconID
	}
	return 0
}

func (x *CreateReq) GetType() pbEnum.AccountType {
	if x != nil {
		return x.Type
	}
	return pbEnum.AccountType(0)
}

func (x *CreateReq) GetCurrency() string {
	if x != nil {
		return x.Currency
	}
	return ""
}

func (x *CreateReq) GetAccountGroupID() uint32 {
	if x != nil {
		return x.AccountGroupID
	}
	return 0
}

func (x *CreateReq) GetAccounting() bool {
	if x != nil {
		return x.Accounting
	}
	return false
}

func (x *CreateReq) GetUserID() uint32 {
	if x != nil {
		return x.UserID
	}
	return 0
}

func (x *CreateReq) GetGradualBudgetFilling() bool {
	if x != nil {
		return x.GradualBudgetFilling
	}
	return false
}

type CreateRes struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	ID uint32 `protobuf:"varint,1,opt,name=ID,proto3" jsonapi:"ID,omitempty"` // Идентификатор созданного счета
}

func (x *CreateRes) Reset() {
	*x = CreateRes{}
	if protoimpl.UnsafeEnabled {
		mi := &file_account_proto_msgTypes[9]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CreateRes) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateRes) ProtoMessage() {}

func (x *CreateRes) ProtoReflect() protoreflect.Message {
	mi := &file_account_proto_msgTypes[9]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateRes.ProtoReflect.Descriptor instead.
func (*CreateRes) Descriptor() ([]byte, []int) {
	return file_account_proto_rawDescGZIP(), []int{9}
}

func (x *CreateRes) GetID() uint32 {
	if x != nil {
		return x.ID
	}
	return 0
}

type GetReq struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	UserID          uint32                `protobuf:"varint,1,opt,name=userID,proto3" jsonapi:"userID,omitempty"`                          // Идентификатор пользователя
	Type            *pbEnum.AccountType   `protobuf:"varint,2,opt,name=type,proto3,enum=enums.AccountType,oneof" jsonapi:"type,omitempty"` // Тип счета
	Accounting      *bool                 `protobuf:"varint,3,opt,name=accounting,proto3,oneof" jsonapi:"accounting,omitempty"`            // Будет ли счет учитываться в статистике
	AccountGroupIDs []uint32              `protobuf:"varint,4,rep,packed,name=accountGroupIDs,proto3" jsonapi:"accountGroupIDs,omitempty"` // Идентификатор группы счета
	DateFrom        *pbDatetime.Timestamp `protobuf:"bytes,6,opt,name=dateFrom,proto3,oneof" jsonapi:"dateFrom,omitempty"`                 // Дата начала выборки (Обязательна при type = expense or earnings и отсутствующем периоде)
	DateTo          *pbDatetime.Timestamp `protobuf:"bytes,7,opt,name=dateTo,proto3,oneof" jsonapi:"dateTo,omitempty"`                     // Дата конца выборки (Обязательна при type = expense or earnings и отсутствующем периоде)
	Visible         *bool                 `protobuf:"varint,8,opt,name=visible,proto3,oneof" jsonapi:"visible,omitempty"`                  // Видимость счета
}

func (x *GetReq) Reset() {
	*x = GetReq{}
	if protoimpl.UnsafeEnabled {
		mi := &file_account_proto_msgTypes[10]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *GetReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetReq) ProtoMessage() {}

func (x *GetReq) ProtoReflect() protoreflect.Message {
	mi := &file_account_proto_msgTypes[10]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetReq.ProtoReflect.Descriptor instead.
func (*GetReq) Descriptor() ([]byte, []int) {
	return file_account_proto_rawDescGZIP(), []int{10}
}

func (x *GetReq) GetUserID() uint32 {
	if x != nil {
		return x.UserID
	}
	return 0
}

func (x *GetReq) GetType() pbEnum.AccountType {
	if x != nil && x.Type != nil {
		return *x.Type
	}
	return pbEnum.AccountType(0)
}

func (x *GetReq) GetAccounting() bool {
	if x != nil && x.Accounting != nil {
		return *x.Accounting
	}
	return false
}

func (x *GetReq) GetAccountGroupIDs() []uint32 {
	if x != nil {
		return x.AccountGroupIDs
	}
	return nil
}

func (x *GetReq) GetDateFrom() *pbDatetime.Timestamp {
	if x != nil {
		return x.DateFrom
	}
	return nil
}

func (x *GetReq) GetDateTo() *pbDatetime.Timestamp {
	if x != nil {
		return x.DateTo
	}
	return nil
}

func (x *GetReq) GetVisible() bool {
	if x != nil && x.Visible != nil {
		return *x.Visible
	}
	return false
}

type UpdateReq struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	UserID               uint32   `protobuf:"varint,1,opt,name=userID,proto3" jsonapi:"userID,omitempty"`                                    // Идентификатор пользователя
	ID                   uint32   `protobuf:"varint,2,opt,name=ID,proto3" jsonapi:"ID,omitempty"`                                            // Идентификатор счета
	Budget               *int32   `protobuf:"varint,3,opt,name=budget,proto3,oneof" jsonapi:"budget,omitempty"`                              // Месячный бюджет
	Remainder            *float64 `protobuf:"fixed64,4,opt,name=remainder,proto3,oneof" jsonapi:"remainder,omitempty"`                       // Остаток средств на счету
	Name                 *string  `protobuf:"bytes,5,opt,name=name,proto3,oneof" jsonapi:"name,omitempty"`                                   // Название счета
	IconID               *uint32  `protobuf:"varint,6,opt,name=iconID,proto3,oneof" jsonapi:"iconID,omitempty"`                              // Идентификатор иконки
	Visible              *bool    `protobuf:"varint,7,opt,name=visible,proto3,oneof" jsonapi:"visible,omitempty"`                            // Видимость счета
	AccountGroupID       *uint32  `protobuf:"varint,8,opt,name=accountGroupID,proto3,oneof" jsonapi:"accountGroupID,omitempty"`              // Идентификатор группы счета
	Accounting           *bool    `protobuf:"varint,9,opt,name=accounting,proto3,oneof" jsonapi:"accounting,omitempty"`                      // Будет ли счет учитываться в статистике
	GradualBudgetFilling *bool    `protobuf:"varint,10,opt,name=gradualBudgetFilling,proto3,oneof" jsonapi:"gradualBudgetFilling,omitempty"` // Постепенное пополнение бюджета
}

func (x *UpdateReq) Reset() {
	*x = UpdateReq{}
	if protoimpl.UnsafeEnabled {
		mi := &file_account_proto_msgTypes[11]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *UpdateReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateReq) ProtoMessage() {}

func (x *UpdateReq) ProtoReflect() protoreflect.Message {
	mi := &file_account_proto_msgTypes[11]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateReq.ProtoReflect.Descriptor instead.
func (*UpdateReq) Descriptor() ([]byte, []int) {
	return file_account_proto_rawDescGZIP(), []int{11}
}

func (x *UpdateReq) GetUserID() uint32 {
	if x != nil {
		return x.UserID
	}
	return 0
}

func (x *UpdateReq) GetID() uint32 {
	if x != nil {
		return x.ID
	}
	return 0
}

func (x *UpdateReq) GetBudget() int32 {
	if x != nil && x.Budget != nil {
		return *x.Budget
	}
	return 0
}

func (x *UpdateReq) GetRemainder() float64 {
	if x != nil && x.Remainder != nil {
		return *x.Remainder
	}
	return 0
}

func (x *UpdateReq) GetName() string {
	if x != nil && x.Name != nil {
		return *x.Name
	}
	return ""
}

func (x *UpdateReq) GetIconID() uint32 {
	if x != nil && x.IconID != nil {
		return *x.IconID
	}
	return 0
}

func (x *UpdateReq) GetVisible() bool {
	if x != nil && x.Visible != nil {
		return *x.Visible
	}
	return false
}

func (x *UpdateReq) GetAccountGroupID() uint32 {
	if x != nil && x.AccountGroupID != nil {
		return *x.AccountGroupID
	}
	return 0
}

func (x *UpdateReq) GetAccounting() bool {
	if x != nil && x.Accounting != nil {
		return *x.Accounting
	}
	return false
}

func (x *UpdateReq) GetGradualBudgetFilling() bool {
	if x != nil && x.GradualBudgetFilling != nil {
		return *x.GradualBudgetFilling
	}
	return false
}

type GetRes struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Accounts []*Account `protobuf:"bytes,1,rep,name=accounts,proto3" jsonapi:"accounts,omitempty"` // Список счетов
}

func (x *GetRes) Reset() {
	*x = GetRes{}
	if protoimpl.UnsafeEnabled {
		mi := &file_account_proto_msgTypes[12]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *GetRes) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetRes) ProtoMessage() {}

func (x *GetRes) ProtoReflect() protoreflect.Message {
	mi := &file_account_proto_msgTypes[12]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetRes.ProtoReflect.Descriptor instead.
func (*GetRes) Descriptor() ([]byte, []int) {
	return file_account_proto_rawDescGZIP(), []int{12}
}

func (x *GetRes) GetAccounts() []*Account {
	if x != nil {
		return x.Accounts
	}
	return nil
}

type Account struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	ID                   uint32             `protobuf:"varint,1,opt,name=ID,proto3" jsonapi:"ID,omitempty"`                                      // Идентификатор счета
	Budget               float64            `protobuf:"fixed64,2,opt,name=budget,proto3" jsonapi:"budget,omitempty"`                             // Месячный бюджет
	Remainder            float64            `protobuf:"fixed64,3,opt,name=remainder,proto3" jsonapi:"remainder,omitempty"`                       // Остаток средств на счету
	Name                 string             `protobuf:"bytes,4,opt,name=name,proto3" jsonapi:"name,omitempty"`                                   // Название счета
	IconID               uint32             `protobuf:"varint,5,opt,name=iconID,proto3" jsonapi:"iconID,omitempty"`                              // Идентификатор иконки
	Type                 pbEnum.AccountType `protobuf:"varint,6,opt,name=type,proto3,enum=enums.AccountType" jsonapi:"type,omitempty"`           // Тип счета
	Currency             string             `protobuf:"bytes,7,opt,name=currency,proto3" jsonapi:"currency,omitempty"`                           // Валюта счета
	Visible              bool               `protobuf:"varint,8,opt,name=visible,proto3" jsonapi:"visible,omitempty"`                            // Видимость счета
	AccountGroupID       uint32             `protobuf:"varint,9,opt,name=accountGroupID,proto3" jsonapi:"accountGroupID,omitempty"`              // Идентификатор группы счета
	Accounting           bool               `protobuf:"varint,10,opt,name=accounting,proto3" jsonapi:"accounting,omitempty"`                     // Будет ли счет учитываться в статистике
	UserID               uint32             `protobuf:"varint,11,opt,name=userID,proto3" jsonapi:"userID,omitempty"`                             // Идентификатор пользователя
	ParentAccountID      *uint32            `protobuf:"varint,12,opt,name=parentAccountID,proto3,oneof" jsonapi:"parentAccountID,omitempty"`     // Идентификатор связи
	ChildrenAccounts     []*Account         `protobuf:"bytes,13,rep,name=childrenAccounts,proto3" jsonapi:"childrenAccounts,omitempty"`          // Дочерние аккаунты
	CurrencySymbol       string             `protobuf:"bytes,14,opt,name=currencySymbol,proto3" jsonapi:"currencySymbol,omitempty"`              // Символ валюты
	GradualBudgetFilling bool               `protobuf:"varint,15,opt,name=gradualBudgetFilling,proto3" jsonapi:"gradualBudgetFilling,omitempty"` // Постепенное пополнение бюджета
	SerialNumber         uint32             `protobuf:"varint,16,opt,name=serialNumber,proto3" jsonapi:"serialNumber,omitempty"`                 // Порядковый номер счета
}

func (x *Account) Reset() {
	*x = Account{}
	if protoimpl.UnsafeEnabled {
		mi := &file_account_proto_msgTypes[13]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Account) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Account) ProtoMessage() {}

func (x *Account) ProtoReflect() protoreflect.Message {
	mi := &file_account_proto_msgTypes[13]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Account.ProtoReflect.Descriptor instead.
func (*Account) Descriptor() ([]byte, []int) {
	return file_account_proto_rawDescGZIP(), []int{13}
}

func (x *Account) GetID() uint32 {
	if x != nil {
		return x.ID
	}
	return 0
}

func (x *Account) GetBudget() float64 {
	if x != nil {
		return x.Budget
	}
	return 0
}

func (x *Account) GetRemainder() float64 {
	if x != nil {
		return x.Remainder
	}
	return 0
}

func (x *Account) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Account) GetIconID() uint32 {
	if x != nil {
		return x.IconID
	}
	return 0
}

func (x *Account) GetType() pbEnum.AccountType {
	if x != nil {
		return x.Type
	}
	return pbEnum.AccountType(0)
}

func (x *Account) GetCurrency() string {
	if x != nil {
		return x.Currency
	}
	return ""
}

func (x *Account) GetVisible() bool {
	if x != nil {
		return x.Visible
	}
	return false
}

func (x *Account) GetAccountGroupID() uint32 {
	if x != nil {
		return x.AccountGroupID
	}
	return 0
}

func (x *Account) GetAccounting() bool {
	if x != nil {
		return x.Accounting
	}
	return false
}

func (x *Account) GetUserID() uint32 {
	if x != nil {
		return x.UserID
	}
	return 0
}

func (x *Account) GetParentAccountID() uint32 {
	if x != nil && x.ParentAccountID != nil {
		return *x.ParentAccountID
	}
	return 0
}

func (x *Account) GetChildrenAccounts() []*Account {
	if x != nil {
		return x.ChildrenAccounts
	}
	return nil
}

func (x *Account) GetCurrencySymbol() string {
	if x != nil {
		return x.CurrencySymbol
	}
	return ""
}

func (x *Account) GetGradualBudgetFilling() bool {
	if x != nil {
		return x.GradualBudgetFilling
	}
	return false
}

func (x *Account) GetSerialNumber() uint32 {
	if x != nil {
		return x.SerialNumber
	}
	return 0
}

var File_account_proto protoreflect.FileDescriptor

var file_account_proto_rawDesc = []byte{
	,
}

var (
	file_account_proto_rawDescOnce sync.Once
	file_account_proto_rawDescData = file_account_proto_rawDesc
)

func file_account_proto_rawDescGZIP() []byte {
	file_account_proto_rawDescOnce.Do(func() {
		file_account_proto_rawDescData = protoimpl.X.CompressGZIP(file_account_proto_rawDescData)
	})
	return file_account_proto_rawDescData
}

var file_account_proto_msgTypes = make([]protoimpl.MessageInfo, 14)
var file_account_proto_goTypes = []interface{}{
	(*GetAccountGroupsReq)(nil),  // 0: pbAccount.GetAccountGroupsReq
	(*GetAccountGroupsRes)(nil),  // 1: pbAccount.GetAccountGroupsRes
	(*AccountGroup)(nil),         // 2: pbAccount.AccountGroup
	(*QuickStatisticReq)(nil),    // 3: pbAccount.QuickStatisticReq
	(*QuickStatisticRes)(nil),    // 4: pbAccount.QuickStatisticRes
	(*QuickStatistic)(nil),       // 5: pbAccount.QuickStatistic
	(*SwitchReq)(nil),            // 6: pbAccount.SwitchReq
	(*DeleteReq)(nil),            // 7: pbAccount.DeleteReq
	(*CreateReq)(nil),            // 8: pbAccount.CreateReq
	(*CreateRes)(nil),            // 9: pbAccount.CreateRes
	(*GetReq)(nil),               // 10: pbAccount.GetReq
	(*UpdateReq)(nil),            // 11: pbAccount.UpdateReq
	(*GetRes)(nil),               // 12: pbAccount.GetRes
	(*Account)(nil),              // 13: pbAccount.Account
	(pbEnum.AccountType)(0),      // 14: enums.AccountType
	(*pbDatetime.Timestamp)(nil), // 15: datetime.Timestamp
	(*emptypb.Empty)(nil),        // 16: google.protobuf.Empty
}
var file_account_proto_depIdxs = []int32{
	2,  // 0: pbAccount.GetAccountGroupsRes.accountGroups:type_name -> pbAccount.AccountGroup
	5,  // 1: pbAccount.QuickStatisticRes.quickStatistic:type_name -> pbAccount.QuickStatistic
	14, // 2: pbAccount.CreateReq.type:type_name -> enums.AccountType
	14, // 3: pbAccount.GetReq.type:type_name -> enums.AccountType
	15, // 4: pbAccount.GetReq.dateFrom:type_name -> datetime.Timestamp
	15, // 5: pbAccount.GetReq.dateTo:type_name -> datetime.Timestamp
	13, // 6: pbAccount.GetRes.accounts:type_name -> pbAccount.Account
	14, // 7: pbAccount.Account.type:type_name -> enums.AccountType
	13, // 8: pbAccount.Account.childrenAccounts:type_name -> pbAccount.Account
	8,  // 9: pbAccount.account.Create:input_type -> pbAccount.CreateReq
	10, // 10: pbAccount.account.Get:input_type -> pbAccount.GetReq
	11, // 11: pbAccount.account.Update:input_type -> pbAccount.UpdateReq
	7,  // 12: pbAccount.account.Delete:input_type -> pbAccount.DeleteReq
	6,  // 13: pbAccount.account.Switch:input_type -> pbAccount.SwitchReq
	3,  // 14: pbAccount.account.QuickStatistic:input_type -> pbAccount.QuickStatisticReq
	0,  // 15: pbAccount.account.GetAccountGroups:input_type -> pbAccount.GetAccountGroupsReq
	9,  // 16: pbAccount.account.Create:output_type -> pbAccount.CreateRes
	12, // 17: pbAccount.account.Get:output_type -> pbAccount.GetRes
	16, // 18: pbAccount.account.Update:output_type -> google.protobuf.Empty
	16, // 19: pbAccount.account.Delete:output_type -> google.protobuf.Empty
	16, // 20: pbAccount.account.Switch:output_type -> google.protobuf.Empty
	4,  // 21: pbAccount.account.QuickStatistic:output_type -> pbAccount.QuickStatisticRes
	1,  // 22: pbAccount.account.GetAccountGroups:output_type -> pbAccount.GetAccountGroupsRes
	16, // [16:23] is the sub-list for method output_type
	9,  // [9:16] is the sub-list for method input_type
	9,  // [9:9] is the sub-list for extension type_name
	9,  // [9:9] is the sub-list for extension extendee
	0,  // [0:9] is the sub-list for field type_name
}

func init() { file_account_proto_init() }
func file_account_proto_init() {
	if File_account_proto != nil {
		return
	}
	if !protoimpl.UnsafeEnabled {
		file_account_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*GetAccountGroupsReq); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_account_proto_msgTypes[1].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*GetAccountGroupsRes); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_account_proto_msgTypes[2].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*AccountGroup); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_account_proto_msgTypes[3].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*QuickStatisticReq); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_account_proto_msgTypes[4].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*QuickStatisticRes); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_account_proto_msgTypes[5].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*QuickStatistic); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_account_proto_msgTypes[6].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*SwitchReq); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_account_proto_msgTypes[7].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*DeleteReq); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_account_proto_msgTypes[8].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CreateReq); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_account_proto_msgTypes[9].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CreateRes); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_account_proto_msgTypes[10].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*GetReq); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_account_proto_msgTypes[11].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*UpdateReq); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_account_proto_msgTypes[12].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*GetRes); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_account_proto_msgTypes[13].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Account); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	file_account_proto_msgTypes[10].OneofWrappers = []interface{}{}
	file_account_proto_msgTypes[11].OneofWrappers = []interface{}{}
	file_account_proto_msgTypes[13].OneofWrappers = []interface{}{}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_account_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   14,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_account_proto_goTypes,
		DependencyIndexes: file_account_proto_depIdxs,
		MessageInfos:      file_account_proto_msgTypes,
	}.Build()
	File_account_proto = out.File
	file_account_proto_rawDesc = nil
	file_account_proto_goTypes = nil
	file_account_proto_depIdxs = nil
}
