// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.28.1
// 	protoc        v4.24.4
// source: transaction.proto

package pbTransaction

import (
	pbEnum "core/app/proto/pbEnum"
	pbDatetime "pkg/proto/pbDatetime"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	emptypb "google.golang.org/protobuf/types/known/emptypb"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type DeleteReq struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	ID     uint32 `protobuf:"varint,1,opt,name=ID,proto3" jsonapi:"ID,omitempty"`         // Идентификатор транзакции
	UserID uint32 `protobuf:"varint,2,opt,name=userID,proto3" jsonapi:"userID,omitempty"` // Идентификатор пользователя
}

func (x *DeleteReq) Reset() {
	*x = DeleteReq{}
	if protoimpl.UnsafeEnabled {
		mi := &file_transaction_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *DeleteReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteReq) ProtoMessage() {}

func (x *DeleteReq) ProtoReflect() protoreflect.Message {
	mi := &file_transaction_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteReq.ProtoReflect.Descriptor instead.
func (*DeleteReq) Descriptor() ([]byte, []int) {
	return file_transaction_proto_rawDescGZIP(), []int{0}
}

func (x *DeleteReq) GetID() uint32 {
	if x != nil {
		return x.ID
	}
	return 0
}

func (x *DeleteReq) GetUserID() uint32 {
	if x != nil {
		return x.UserID
	}
	return 0
}

type CreateReq struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Type            pbEnum.TransactionType `protobuf:"varint,1,opt,name=type,proto3,enum=enums.TransactionType" jsonapi:"type,omitempty"` // Тип транзакции
	AmountFrom      float64                `protobuf:"fixed64,2,opt,name=amountFrom,proto3" jsonapi:"amountFrom,omitempty"`               // Сумма списания с первого счета
	AmountTo        float64                `protobuf:"fixed64,3,opt,name=amountTo,proto3" jsonapi:"amountTo,omitempty"`                   // Сумма пополнения второго счета (в случаях меж валютной транзакции цифры отличаются)
	Note            string                 `protobuf:"bytes,4,opt,name=note,proto3" jsonapi:"note,omitempty"`                             // Заметка для транзакции
	AccountFromID   uint32                 `protobuf:"varint,5,opt,name=accountFromID,proto3" jsonapi:"accountFromID,omitempty"`          // Идентификатор счета списания
	AccountToID     uint32                 `protobuf:"varint,6,opt,name=accountToID,proto3" jsonapi:"accountToID,omitempty"`              // Идентификатор счета пополнения
	DateTransaction *pbDatetime.Timestamp  `protobuf:"bytes,7,opt,name=dateTransaction,proto3" jsonapi:"dateTransaction,omitempty"`       // Дата транзакции
	IsExecuted      *bool                  `protobuf:"varint,8,opt,name=isExecuted,proto3,oneof" jsonapi:"isExecuted,omitempty"`          // Исполнена операция или нет (если нет, сделки как бы не существует)
	UserID          uint32                 `protobuf:"varint,9,opt,name=userID,proto3" jsonapi:"userID,omitempty"`                        // Идентификатор пользователя
}

func (x *CreateReq) Reset() {
	*x = CreateReq{}
	if protoimpl.UnsafeEnabled {
		mi := &file_transaction_proto_msgTypes[1]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CreateReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateReq) ProtoMessage() {}

func (x *CreateReq) ProtoReflect() protoreflect.Message {
	mi := &file_transaction_proto_msgTypes[1]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateReq.ProtoReflect.Descriptor instead.
func (*CreateReq) Descriptor() ([]byte, []int) {
	return file_transaction_proto_rawDescGZIP(), []int{1}
}

func (x *CreateReq) GetType() pbEnum.TransactionType {
	if x != nil {
		return x.Type
	}
	return pbEnum.TransactionType(0)
}

func (x *CreateReq) GetAmountFrom() float64 {
	if x != nil {
		return x.AmountFrom
	}
	return 0
}

func (x *CreateReq) GetAmountTo() float64 {
	if x != nil {
		return x.AmountTo
	}
	return 0
}

func (x *CreateReq) GetNote() string {
	if x != nil {
		return x.Note
	}
	return ""
}

func (x *CreateReq) GetAccountFromID() uint32 {
	if x != nil {
		return x.AccountFromID
	}
	return 0
}

func (x *CreateReq) GetAccountToID() uint32 {
	if x != nil {
		return x.AccountToID
	}
	return 0
}

func (x *CreateReq) GetDateTransaction() *pbDatetime.Timestamp {
	if x != nil {
		return x.DateTransaction
	}
	return nil
}

func (x *CreateReq) GetIsExecuted() bool {
	if x != nil && x.IsExecuted != nil {
		return *x.IsExecuted
	}
	return false
}

func (x *CreateReq) GetUserID() uint32 {
	if x != nil {
		return x.UserID
	}
	return 0
}

type CreateRes struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	ID uint32 `protobuf:"varint,1,opt,name=ID,proto3" jsonapi:"ID,omitempty"` // Идентификатор транзакции
}

func (x *CreateRes) Reset() {
	*x = CreateRes{}
	if protoimpl.UnsafeEnabled {
		mi := &file_transaction_proto_msgTypes[2]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CreateRes) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateRes) ProtoMessage() {}

func (x *CreateRes) ProtoReflect() protoreflect.Message {
	mi := &file_transaction_proto_msgTypes[2]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateRes.ProtoReflect.Descriptor instead.
func (*CreateRes) Descriptor() ([]byte, []int) {
	return file_transaction_proto_rawDescGZIP(), []int{2}
}

func (x *CreateRes) GetID() uint32 {
	if x != nil {
		return x.ID
	}
	return 0
}

type UpdateReq struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	ID              uint32                `protobuf:"varint,1,opt,name=ID,proto3" jsonapi:"ID,omitempty"`                                // Идентификатор транзакции
	UserID          uint32                `protobuf:"varint,2,opt,name=userID,proto3" jsonapi:"userID,omitempty"`                        // Идентификатор пользователя
	AmountFrom      *float64              `protobuf:"fixed64,3,opt,name=amountFrom,proto3,oneof" jsonapi:"amountFrom,omitempty"`         // Сумма списания с первого счета
	AmountTo        *float64              `protobuf:"fixed64,4,opt,name=amountTo,proto3,oneof" jsonapi:"amountTo,omitempty"`             // Сумма пополнения второго счета
	Note            *string               `protobuf:"bytes,5,opt,name=note,proto3,oneof" jsonapi:"note,omitempty"`                       // Заметка для транзакции
	AccountFromID   *uint32               `protobuf:"varint,6,opt,name=accountFromID,proto3,oneof" jsonapi:"accountFromID,omitempty"`    // Идентификатор счета списания
	AccountToID     *uint32               `protobuf:"varint,7,opt,name=accountToID,proto3,oneof" jsonapi:"accountToID,omitempty"`        // Идентификатор счета пополнения
	DateTransaction *pbDatetime.Timestamp `protobuf:"bytes,8,opt,name=dateTransaction,proto3,oneof" jsonapi:"dateTransaction,omitempty"` // Дата транзакции
	IsExecuted      *bool                 `protobuf:"varint,9,opt,name=isExecuted,proto3,oneof" jsonapi:"isExecuted,omitempty"`          // Исполнена операция или нет (если нет, сделки как бы не существует)
}

func (x *UpdateReq) Reset() {
	*x = UpdateReq{}
	if protoimpl.UnsafeEnabled {
		mi := &file_transaction_proto_msgTypes[3]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *UpdateReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateReq) ProtoMessage() {}

func (x *UpdateReq) ProtoReflect() protoreflect.Message {
	mi := &file_transaction_proto_msgTypes[3]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateReq.ProtoReflect.Descriptor instead.
func (*UpdateReq) Descriptor() ([]byte, []int) {
	return file_transaction_proto_rawDescGZIP(), []int{3}
}

func (x *UpdateReq) GetID() uint32 {
	if x != nil {
		return x.ID
	}
	return 0
}

func (x *UpdateReq) GetUserID() uint32 {
	if x != nil {
		return x.UserID
	}
	return 0
}

func (x *UpdateReq) GetAmountFrom() float64 {
	if x != nil && x.AmountFrom != nil {
		return *x.AmountFrom
	}
	return 0
}

func (x *UpdateReq) GetAmountTo() float64 {
	if x != nil && x.AmountTo != nil {
		return *x.AmountTo
	}
	return 0
}

func (x *UpdateReq) GetNote() string {
	if x != nil && x.Note != nil {
		return *x.Note
	}
	return ""
}

func (x *UpdateReq) GetAccountFromID() uint32 {
	if x != nil && x.AccountFromID != nil {
		return *x.AccountFromID
	}
	return 0
}

func (x *UpdateReq) GetAccountToID() uint32 {
	if x != nil && x.AccountToID != nil {
		return *x.AccountToID
	}
	return 0
}

func (x *UpdateReq) GetDateTransaction() *pbDatetime.Timestamp {
	if x != nil {
		return x.DateTransaction
	}
	return nil
}

func (x *UpdateReq) GetIsExecuted() bool {
	if x != nil && x.IsExecuted != nil {
		return *x.IsExecuted
	}
	return false
}

type GetReq struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	UserID    uint32                  `protobuf:"varint,1,opt,name=userID,proto3" jsonapi:"userID,omitempty"`                              // Идентификатор пользователя
	Offset    *uint32                 `protobuf:"varint,2,opt,name=offset,proto3,oneof" jsonapi:"offset,omitempty"`                        // Смещение для пагинации
	Limit     *uint32                 `protobuf:"varint,3,opt,name=limit,proto3,oneof" jsonapi:"limit,omitempty"`                          // Лимит для пагинации
	AccountID *uint32                 `protobuf:"varint,4,opt,name=accountID,proto3,oneof" jsonapi:"accountID,omitempty"`                  // Транзакции какого счета нас интересуют
	Type      *pbEnum.TransactionType `protobuf:"varint,5,opt,name=type,proto3,enum=enums.TransactionType,oneof" jsonapi:"type,omitempty"` // Тип транзакции
	DateFrom  *pbDatetime.Timestamp   `protobuf:"bytes,6,opt,name=dateFrom,proto3,oneof" jsonapi:"dateFrom,omitempty"`                     // Дата, от которой начинать учитывать транзакции
	DateTo    *pbDatetime.Timestamp   `protobuf:"bytes,7,opt,name=dateTo,proto3,oneof" jsonapi:"dateTo,omitempty"`                         // Дата, до которой учитывать транзакции
}

func (x *GetReq) Reset() {
	*x = GetReq{}
	if protoimpl.UnsafeEnabled {
		mi := &file_transaction_proto_msgTypes[4]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *GetReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetReq) ProtoMessage() {}

func (x *GetReq) ProtoReflect() protoreflect.Message {
	mi := &file_transaction_proto_msgTypes[4]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetReq.ProtoReflect.Descriptor instead.
func (*GetReq) Descriptor() ([]byte, []int) {
	return file_transaction_proto_rawDescGZIP(), []int{4}
}

func (x *GetReq) GetUserID() uint32 {
	if x != nil {
		return x.UserID
	}
	return 0
}

func (x *GetReq) GetOffset() uint32 {
	if x != nil && x.Offset != nil {
		return *x.Offset
	}
	return 0
}

func (x *GetReq) GetLimit() uint32 {
	if x != nil && x.Limit != nil {
		return *x.Limit
	}
	return 0
}

func (x *GetReq) GetAccountID() uint32 {
	if x != nil && x.AccountID != nil {
		return *x.AccountID
	}
	return 0
}

func (x *GetReq) GetType() pbEnum.TransactionType {
	if x != nil && x.Type != nil {
		return *x.Type
	}
	return pbEnum.TransactionType(0)
}

func (x *GetReq) GetDateFrom() *pbDatetime.Timestamp {
	if x != nil {
		return x.DateFrom
	}
	return nil
}

func (x *GetReq) GetDateTo() *pbDatetime.Timestamp {
	if x != nil {
		return x.DateTo
	}
	return nil
}

type GetRes struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Transactions []*Transaction `protobuf:"bytes,1,rep,name=transactions,proto3" jsonapi:"transactions,omitempty"`
}

func (x *GetRes) Reset() {
	*x = GetRes{}
	if protoimpl.UnsafeEnabled {
		mi := &file_transaction_proto_msgTypes[5]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *GetRes) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetRes) ProtoMessage() {}

func (x *GetRes) ProtoReflect() protoreflect.Message {
	mi := &file_transaction_proto_msgTypes[5]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetRes.ProtoReflect.Descriptor instead.
func (*GetRes) Descriptor() ([]byte, []int) {
	return file_transaction_proto_rawDescGZIP(), []int{5}
}

func (x *GetRes) GetTransactions() []*Transaction {
	if x != nil {
		return x.Transactions
	}
	return nil
}

type Transaction struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	ID              uint32                 `protobuf:"varint,1,opt,name=ID,proto3" jsonapi:"ID,omitempty"`                                // Идентификатор транзакции
	Type            pbEnum.TransactionType `protobuf:"varint,2,opt,name=type,proto3,enum=enums.TransactionType" jsonapi:"type,omitempty"` // Тип транзакции
	Tags            []*Tag                 `protobuf:"bytes,3,rep,name=tags,proto3" jsonapi:"tags,omitempty"`                             // Подкатегории
	AmountFrom      float64                `protobuf:"fixed64,4,opt,name=amountFrom,proto3" jsonapi:"amountFrom,omitempty"`               // Сумма сделки в первой валюте
	AmountTo        float64                `protobuf:"fixed64,5,opt,name=amountTo,proto3" jsonapi:"amountTo,omitempty"`                   // Сумма сделки во второй валюте
	Note            string                 `protobuf:"bytes,6,opt,name=note,proto3" jsonapi:"note,omitempty"`                             // Заметка сделки
	AccountFromID   uint32                 `protobuf:"varint,7,opt,name=accountFromID,proto3" jsonapi:"accountFromID,omitempty"`          // Идентификатор счета списания
	AccountToID     uint32                 `protobuf:"varint,8,opt,name=accountToID,proto3" jsonapi:"accountToID,omitempty"`              // Идентификатор счета пополнения
	DateTransaction *pbDatetime.Timestamp  `protobuf:"bytes,9,opt,name=dateTransaction,proto3" jsonapi:"dateTransaction,omitempty"`       // Дата транзакции (пользовательские)
	IsExecuted      bool                   `protobuf:"varint,10,opt,name=isExecuted,proto3" jsonapi:"isExecuted,omitempty"`               // Исполнена операция или нет (если нет, сделки как бы не существует)
	Accounting      bool                   `protobuf:"varint,11,opt,name=accounting,proto3" jsonapi:"accounting,omitempty"`               // Учитывается ли транзакция в статистике или нет
	TimeCreate      *pbDatetime.Timestamp  `protobuf:"bytes,12,opt,name=timeCreate,proto3,oneof" jsonapi:"timeCreate,omitempty"`          // Дата и время создания транзакции
}

func (x *Transaction) Reset() {
	*x = Transaction{}
	if protoimpl.UnsafeEnabled {
		mi := &file_transaction_proto_msgTypes[6]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Transaction) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Transaction) ProtoMessage() {}

func (x *Transaction) ProtoReflect() protoreflect.Message {
	mi := &file_transaction_proto_msgTypes[6]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Transaction.ProtoReflect.Descriptor instead.
func (*Transaction) Descriptor() ([]byte, []int) {
	return file_transaction_proto_rawDescGZIP(), []int{6}
}

func (x *Transaction) GetID() uint32 {
	if x != nil {
		return x.ID
	}
	return 0
}

func (x *Transaction) GetType() pbEnum.TransactionType {
	if x != nil {
		return x.Type
	}
	return pbEnum.TransactionType(0)
}

func (x *Transaction) GetTags() []*Tag {
	if x != nil {
		return x.Tags
	}
	return nil
}

func (x *Transaction) GetAmountFrom() float64 {
	if x != nil {
		return x.AmountFrom
	}
	return 0
}

func (x *Transaction) GetAmountTo() float64 {
	if x != nil {
		return x.AmountTo
	}
	return 0
}

func (x *Transaction) GetNote() string {
	if x != nil {
		return x.Note
	}
	return ""
}

func (x *Transaction) GetAccountFromID() uint32 {
	if x != nil {
		return x.AccountFromID
	}
	return 0
}

func (x *Transaction) GetAccountToID() uint32 {
	if x != nil {
		return x.AccountToID
	}
	return 0
}

func (x *Transaction) GetDateTransaction() *pbDatetime.Timestamp {
	if x != nil {
		return x.DateTransaction
	}
	return nil
}

func (x *Transaction) GetIsExecuted() bool {
	if x != nil {
		return x.IsExecuted
	}
	return false
}

func (x *Transaction) GetAccounting() bool {
	if x != nil {
		return x.Accounting
	}
	return false
}

func (x *Transaction) GetTimeCreate() *pbDatetime.Timestamp {
	if x != nil {
		return x.TimeCreate
	}
	return nil
}

type Tag struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	TransactionID uint32 `protobuf:"varint,1,opt,name=transactionID,proto3" jsonapi:"transactionID,omitempty"` // Идентификатор транзакции
	TagID         uint32 `protobuf:"varint,2,opt,name=tagID,proto3" jsonapi:"tagID,omitempty"`                 // Идентификатор подкатегории
}

func (x *Tag) Reset() {
	*x = Tag{}
	if protoimpl.UnsafeEnabled {
		mi := &file_transaction_proto_msgTypes[7]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Tag) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Tag) ProtoMessage() {}

func (x *Tag) ProtoReflect() protoreflect.Message {
	mi := &file_transaction_proto_msgTypes[7]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Tag.ProtoReflect.Descriptor instead.
func (*Tag) Descriptor() ([]byte, []int) {
	return file_transaction_proto_rawDescGZIP(), []int{7}
}

func (x *Tag) GetTransactionID() uint32 {
	if x != nil {
		return x.TransactionID
	}
	return 0
}

func (x *Tag) GetTagID() uint32 {
	if x != nil {
		return x.TagID
	}
	return 0
}

var File_transaction_proto protoreflect.FileDescriptor

var file_transaction_proto_rawDesc = []byte{
	,
}

var (
	file_transaction_proto_rawDescOnce sync.Once
	file_transaction_proto_rawDescData = file_transaction_proto_rawDesc
)

func file_transaction_proto_rawDescGZIP() []byte {
	file_transaction_proto_rawDescOnce.Do(func() {
		file_transaction_proto_rawDescData = protoimpl.X.CompressGZIP(file_transaction_proto_rawDescData)
	})
	return file_transaction_proto_rawDescData
}

var file_transaction_proto_msgTypes = make([]protoimpl.MessageInfo, 8)
var file_transaction_proto_goTypes = []interface{}{
	(*DeleteReq)(nil),            // 0: pbTransaction.DeleteReq
	(*CreateReq)(nil),            // 1: pbTransaction.CreateReq
	(*CreateRes)(nil),            // 2: pbTransaction.CreateRes
	(*UpdateReq)(nil),            // 3: pbTransaction.UpdateReq
	(*GetReq)(nil),               // 4: pbTransaction.GetReq
	(*GetRes)(nil),               // 5: pbTransaction.GetRes
	(*Transaction)(nil),          // 6: pbTransaction.Transaction
	(*Tag)(nil),                  // 7: pbTransaction.Tag
	(pbEnum.TransactionType)(0),  // 8: enums.TransactionType
	(*pbDatetime.Timestamp)(nil), // 9: datetime.Timestamp
	(*emptypb.Empty)(nil),        // 10: google.protobuf.Empty
}
var file_transaction_proto_depIdxs = []int32{
	8,  // 0: pbTransaction.CreateReq.type:type_name -> enums.TransactionType
	9,  // 1: pbTransaction.CreateReq.dateTransaction:type_name -> datetime.Timestamp
	9,  // 2: pbTransaction.UpdateReq.dateTransaction:type_name -> datetime.Timestamp
	8,  // 3: pbTransaction.GetReq.type:type_name -> enums.TransactionType
	9,  // 4: pbTransaction.GetReq.dateFrom:type_name -> datetime.Timestamp
	9,  // 5: pbTransaction.GetReq.dateTo:type_name -> datetime.Timestamp
	6,  // 6: pbTransaction.GetRes.transactions:type_name -> pbTransaction.Transaction
	8,  // 7: pbTransaction.Transaction.type:type_name -> enums.TransactionType
	7,  // 8: pbTransaction.Transaction.tags:type_name -> pbTransaction.Tag
	9,  // 9: pbTransaction.Transaction.dateTransaction:type_name -> datetime.Timestamp
	9,  // 10: pbTransaction.Transaction.timeCreate:type_name -> datetime.Timestamp
	1,  // 11: pbTransaction.transaction.Create:input_type -> pbTransaction.CreateReq
	3,  // 12: pbTransaction.transaction.Update:input_type -> pbTransaction.UpdateReq
	0,  // 13: pbTransaction.transaction.Delete:input_type -> pbTransaction.DeleteReq
	4,  // 14: pbTransaction.transaction.Get:input_type -> pbTransaction.GetReq
	2,  // 15: pbTransaction.transaction.Create:output_type -> pbTransaction.CreateRes
	10, // 16: pbTransaction.transaction.Update:output_type -> google.protobuf.Empty
	10, // 17: pbTransaction.transaction.Delete:output_type -> google.protobuf.Empty
	5,  // 18: pbTransaction.transaction.Get:output_type -> pbTransaction.GetRes
	15, // [15:19] is the sub-list for method output_type
	11, // [11:15] is the sub-list for method input_type
	11, // [11:11] is the sub-list for extension type_name
	11, // [11:11] is the sub-list for extension extendee
	0,  // [0:11] is the sub-list for field type_name
}

func init() { file_transaction_proto_init() }
func file_transaction_proto_init() {
	if File_transaction_proto != nil {
		return
	}
	if !protoimpl.UnsafeEnabled {
		file_transaction_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*DeleteReq); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_transaction_proto_msgTypes[1].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CreateReq); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_transaction_proto_msgTypes[2].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CreateRes); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_transaction_proto_msgTypes[3].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*UpdateReq); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_transaction_proto_msgTypes[4].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*GetReq); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_transaction_proto_msgTypes[5].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*GetRes); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_transaction_proto_msgTypes[6].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Transaction); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_transaction_proto_msgTypes[7].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Tag); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	file_transaction_proto_msgTypes[1].OneofWrappers = []interface{}{}
	file_transaction_proto_msgTypes[3].OneofWrappers = []interface{}{}
	file_transaction_proto_msgTypes[4].OneofWrappers = []interface{}{}
	file_transaction_proto_msgTypes[6].OneofWrappers = []interface{}{}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_transaction_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   8,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_transaction_proto_goTypes,
		DependencyIndexes: file_transaction_proto_depIdxs,
		MessageInfos:      file_transaction_proto_msgTypes,
	}.Build()
	File_transaction_proto = out.File
	file_transaction_proto_rawDesc = nil
	file_transaction_proto_goTypes = nil
	file_transaction_proto_depIdxs = nil
}
